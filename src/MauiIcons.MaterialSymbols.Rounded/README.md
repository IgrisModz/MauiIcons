...
This library provides **MaterialSymbols Rounded** support for **.NET MAUI** (.NET 10). It allows you to use icons as fonts, offering full control over size, color, and animations with high performance.
...
```xml
<mi:MaterialSymbolsRoundedIcon Icon="Heart" 
                         TextColor="Red" 
                         FontSize="40" 
                         Animation="Pulse" 
...
```xml
<Label Text="{mi:MaterialSymbolsRounded Icon=Home, Size=60, Color=DarkBlue}" />
<Image Source="{mi:MaterialSymbolsRounded Icon=Image, Size=50}" />
```

### Platform-Specific Icons

```xml
<mi:MaterialSymbolsRoundedIcon Icon="{mi:MaterialSymbolsRoundedPlatform WinUI=Image, Android=Home}" />
```
